...
class MainUI extends Component {
  ...
  fetchCurrentConditions() {
    ...
    }?ACCUWE&apikey=rvSVrxAEhkTPZ8Zzou6hLusbiaZAobB9`;
    return fetch(url)
  ...
  fetchZipCode = () => {
    ...
    fetch(zipCodeURL)
      ...
      .then(zipCodeResponse => {
        ...
        const usLocations = zipCodeResponse.filter(
          location => location.Country.ID === "US"
        );
  ...
  renderLoadingIcon() {
    ...
    return <Loader type="Triangle" color="black" height={80} width={80} />;
  }
  renderZipForm() {
  ...
  renderContentArea() {
    const { currentTemp, currentConditions, reactionURL } = this.state;
    ...
    return (
      <div className={styles.contentArea}>
        ...
        <div className={styles.conditionsArea}>
          <div className={styles.tempurature}>{currentTemp}</div>
          <div className={styles.condition}>{currentConditions}</div>
        </div>
      </div>
    );
  }
